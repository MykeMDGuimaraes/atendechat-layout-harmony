...
const CustomToolTip: React.FC<CustomToolTipProps> = ({ title, content, children }) => {
...
const Connections: React.FC = () => {
  ...
  const handleStartWhatsAppSession = async (whatsAppId: number) => {
    try {
  ...
  const handleRequestNewQrCode = async (whatsAppId: number) => {
    try {
  ...
  const handleOpenConfirmationModal = (action: string, whatsAppId: number) => {
  ...
  const handleSubmitConfirmationModal = async () => {
    ...
    setConfirmModalInfo({ action: "", title: "", message: "", whatsAppId: 0 });
  };
